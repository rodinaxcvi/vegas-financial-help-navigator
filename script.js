...
function resourceMatches(resource) {
  ...
  return (category === "Any" || resource.category === category)
    && (transport !== "Bus preferred" || resource.busAccessible)
...
function resourceMatchesLocation(resource, search) {
  if (!search) return true;
  const haystack = [resource.address, resource.organization, ...(resource.neighborhoods || [])].join(" ").toLowerCase();
  return haystack.includes(search);
}

function renderResources() {
  const search = $("zipNeighborhood").value.trim().toLowerCase();
  const categoryMatches = resources.filter(resourceMatches);
  const locationMatches = categoryMatches.filter((resource) => resourceMatchesLocation(resource, search));
  if (locationMatches.length) {
    $("resourceCards").innerHTML = locationMatches.map(resourceCard).join("");
    return;
  }
  if (categoryMatches.length) {
    $("resourceCards").innerHTML = [
      regionalResultsCard(search),
      ...categoryMatches.map(resourceCard)
    ].join("");
    return;
  }
...
function regionalResultsCard(search) {
  return `<article class="resource-card info-card">
    <span class="verified-pill">Regional results</span>
    <h3>No ZIP-specific match listed${search ? ` for ${escapeHTML(search)}` : ""}</h3>
    <p>Showing verified regional starting points that may help with the selected need. Confirm availability, service area, eligibility, and hours directly with the organization before traveling or applying.</p>
  </article>`;
}
...
function bindEvents() {
  ...
  document.querySelectorAll(".tab").forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      localStorage.setItem(STORAGE.activeSection, tab.dataset.section);
  ...
  ["resourceFilters", "resourceCategory", "zipNeighborhood", "transportAvailable", "householdType", "resourceUrgency"].forEach((id) => {
